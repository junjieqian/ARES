...
dacapobenchmark=(["eclipse"]="330m" ["jython"]="90m" ["lusearch"]="90m" ["pmd"]="210m" \
  ["sunflow"]="210m" ["avrora"]="75m" ["xalan"]="150m" ["h2"]="900m" ["tomcat"]="135m")

declare -A specbenchmark
#specbenchmark=(["compiler.sunflow"]="6000m" ["crypto.aes"]="2000m" 
#  ["scimark.fft.large"]="4000m" ["xml.validation"]="2000m" ["xml.transform"]="2000m")
specbenchmark=(["compiler.compiler"]="2000m" ["compress"]="1000m" ["crypto.signverify"]="1500m" \
  ["mpegaudio"]="1500m" ["compiler.sunflow"]="6000m" ["crypto.aes"]="2000m" \
  ["scimark.fft.large"]="4000m" ["xml.validation"]="2000m" ["xml.transform"]="2000m")
# ["derby"]="3000m")
...
do
  ...
  do
    ...
    python /home/jqian/multi-threading/adpative-scheduler/concurrent_run_Hotspots.py java \
    -Xms${dacapobenchmark["$name"]} -Xmx${dacapobenchmark["$name"]} -verbose:gc \
    -XX:+PrintGCDetails -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime \
    -XX:+UseParallelGC -Xloggc:$LOGFILE"$name"_"$thread"_"$i" -jar $Dacapo $name -t 64 >> \
    $LOGFILE"$name"_"$thread"
          echo "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" >> \
          $LOGFILE"$name"_"$thread"
    done
  done
...
do
   ...
   for i in {1..3}; do
      for name in "${!specbenchmark[@]}"; do
        ...
        python /home/jqian/multi-threading/adpative-scheduler/concurrent_run_Hotspots.py java \
        -Xms${specbenchmark["$name"]} -Xmx${specbenchmark["$name"]} -verbose:gc \
        -XX:+PrintGCDetails -XX:+PrintGCApplicationConcurrentTime -XX:+PrintGCApplicationStoppedTime \
        -XX:+UseParallelGC -Xloggc:$LOGFILE"$name"_"$thread"_"$i" -jar $SPECJVM_jar $name -bt 64 \
        -ops 10 &>> $LOGFILE"$name"_"$thread"
        echo "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" >> \
        $LOGFILE"$name"_"$thread"
      done
   done
